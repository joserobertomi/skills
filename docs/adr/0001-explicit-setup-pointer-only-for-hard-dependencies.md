...
- **Hard dependency** (`to-issues`, `to-prd`, `triage`, `english-artifacts`) — include an explicit one-liner: _"… should have been provided to you — run `/setup-matt-pocock-skills` if not."_ Without the mapping, output is wrong, not just fuzzy. (`english-artifacts` falls back to a documented default of English artifacts + pt-BR conversation, but still needs `docs/agents/artifact-language.md` to know any other project's conversation language.)
