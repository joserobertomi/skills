...
- **Hard dependency** (`to-issues`, `to-prd`, `triage`, `english-artifacts`) — include an explicit one-liner: _"… should have been provided to you — run `/setup-matt-pocock-skills` if not."_ Without the mapping, output is wrong, not just fuzzy. (`english-artifacts` is the exception that self-bootstraps `docs/agents/artifact-language.md` rather than only defaulting silently — see [[0003]].)
